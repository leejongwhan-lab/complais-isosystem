...
const EXT_COLORS: Record<string, { color: string; icon: string }> = {
  ...
  pptx: { color: "#E67700", icon: "📑" },
  ppt:  { color: "#E67700", icon: "📑" },
};
...
export default function ManualsClient() {
  ...
  useEffect(() => {
    async function load() {
      const [docsRes, filesRes] = await Promise.all([
        supabase
          .from("documents")
          .select("id, doc_number, title, version, status, owner_name, updated_at")
          .eq("doc_type", "M")
          .order("updated_at", { ascending: false }),
        supabase
          .from("document_files")
          .select("id, file_name, file_size, file_ext, file_url, description, version, uploaded_by, created_at")
          .eq("doc_type", "manual")
  ...
  async function handleUpload(fileList: FileList | null) {
    ...
    for (const file of Array.from(fileList)) {
      ...
      const path = `manuals/${Date.now()}_${file.name}`;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 52px)" }}>
      <div style={{
        ...
        <div style={{ display: "flex" }}>
          {([
            { key: "docs" as TabKey,   label: "등록된 문서" },
            { key: "files" as TabKey,  label: "업로드 파일" },
            { key: "upload" as TabKey, label: "파일 업로드" },
          ]).map(t => (
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
        {tab === "docs" && (
          <>
            <div style={{ marginBottom: 14 }}>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#1a1a1a" }}>매뉴얼</h2>
              <p style={{ margin: "3px 0 0", fontSize: 12, color: "#999" }}>
                doc_type = M · 품질매뉴얼, 환경매뉴얼 등 최상위 문서
              </p>
            </div>
            {loading ? (
              ...
              <div style={{ textAlign: "center", padding: "60px 0" }}>
                ...
                <p style={{ marginTop: 12, color: "#bbb", fontSize: 13 }}>
                  등록된 매뉴얼 문서가 없습니다.<br />
                  <Link href="/documents/new?type=M" style={{ color: "#3B5BDB" }}>새로 작성하기</Link>
                </p>
              </div>
            ) : (
              <div style={{ border: "1px solid #E5E5E5", borderRadius: 8, overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead style={{ background: "#FAFAFA", borderBottom: "1px solid #E5E5E5" }}>
                    <tr>
                      {["문서번호", "문서명", "버전", "상태", "담당자", "최종수정"].map((h, i) => (
                  ...
                  <tbody>
                    {docRecords.map((doc, idx) => {
                      ...
                      return (
                        ...
                        >
                          <td style={{ padding: "10px 14px", fontSize: 12, fontFamily: "monospace", color: "#555" }}>{doc.doc_number}</td>
                          <td style={{ padding: "10px 14px", fontSize: 13, color: "#1a1a1a" }}>{doc.title}</td>
            ...
            {loading ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
                {files.map(file => {
                  ...
                  return (
                    <div key={file.id} style={{ border: "1px solid #E5E5E5", borderRadius: 10, padding: "14px", background: "#fff", display: "flex", flexDirection: "column", gap: 8 }}>
                      <div style={{ fontSize: 32, textAlign: "center" }}>{extInfo.icon}</div>
                      <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: "#1a1a1a", textAlign: "center", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {file.file_name}
                      </p>
                      <p style={{ margin: 0, fontSize: 11, color: "#aaa", textAlign: "center" }}>버전: {file.version ?? "R00"}</p>
                      {file.description && <p style={{ margin: 0, fontSize: 11, color: "#888", textAlign: "center" }}>{file.description}</p>}
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button onClick={() => handleDelete(file)} style={{
                          ...
                          border: "1px solid #FFD0D0", background: "#FFF5F5", color: "#E03131", cursor: "pointer",
                        }}>
                          <Trash2 size={11} />
                        </button>
                      </div>
        ...
        {tab === "upload" && (
          <div style={{ maxWidth: 560, margin: "0 auto" }}>
            <h2 style={{ margin: "0 0 4px", fontSize: 16, fontWeight: 700, color: "#1a1a1a" }}>매뉴얼 파일 업로드</h2>
            <p style={{ margin: "0 0 24px", fontSize: 12, color: "#999" }}>PDF, Word, HWP 등 파일을 업로드합니다.</p>
            <div
            ...
            >
              ...
              <p style={{ margin: "8px 0 0", fontSize: 12, color: "#999" }}>PDF, DOCX, HWP, XLSX 등 모든 파일 형식 지원</p>
